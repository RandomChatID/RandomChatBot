...
token = "2119285705:AAFvhvmN7tMzPqqxvHaHp5i32UGkjM9uy1s"
...
def handle(update):
	...
	try:
		...
		if uid in queue["occupied"]:
			if 'text' in update:
				if text != "/end":
				        bot.sendMessage(queue["occupied"][uid], "🌐 Stranger Online 🗣️: " + text)
		...
		if len(queue["free"]) > 1 and not uid in queue["occupied"]:
			...
			if partner != uid:
				...
				bot.sendMessage(uid, 'Anda telah cocok, silahkan berbicara dengan pasanganmu ♿!')
				bot.sendMessage(partner, 'Anda telah cocok, silahkan berbicara dengan pasanganmu ♿!')
